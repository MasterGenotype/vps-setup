...
# tunnel (WireGuard VPN) server, with encrypted DNS (DNSCrypt-Proxy), basic
# hardening, intrusion detection & prevention (Suricata IDPS), and initial
# client configs for a computer and a phone.
#
# Skip pieces:
#   sudo NO_CLIENTS=1 ./bootstrap.sh
#   sudo NO_DNSCRYPT=1 ./bootstrap.sh
#   sudo NO_SURICATA=1 ./bootstrap.sh
...
if [[ -z ${NO_DNSCRYPT:-} ]]; then
    echo "Check encrypted DNS with:"
    echo "  systemctl status dnscrypt-proxy"
fi
